...
*)
type 'conj t = TRUE | DISJ of 'conj list
...
let map (f : 'a -> 'b) (expr : 'a t) : 'b t =
  match expr with TRUE -> TRUE | DISJ l -> DISJ (List.map f l)
...
let make_cst b = if b then TRUE else DISJ []
...
let make_not ~cand ~cnegate (expr : 'a t) : 'a t =
  match expr with
  | TRUE -> DISJ []
  | DISJ l ->
      List.fold_left
        (fun res conjunction -> make_and ~cand res (cnegate conjunction))
        TRUE l
